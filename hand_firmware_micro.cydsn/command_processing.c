...
void get_param_list(uint16 index) {
    ...
    uint8 packet_data[1851] = "";
    uint16 packet_lenght = 1851;
    ...
    char handle_grasp_time_str[30] = "26 - Handle Grasp Time [sec]:";
    char handle_grasp_thr_str[38] = "27 - Handle Grasp Thresholds [ticks]:";
    ...
    uint8 CYDATA handle_grasp_time_str_len = strlen(handle_grasp_time_str);
    uint8 CYDATA handle_grasp_thr_str_len = strlen(handle_grasp_thr_str);
    ...
    switch(index) {
        case 0:         //List of all parameters with relative types
            ...
            *((int8 *)(packet_data + 1154)) = hand_comp_calib.repetitions;
            ...
            packet_data[1253] = 1;
            *((float *)(packet_data + 1254)) = c_mem.handle_grasp_time;
            for(i = handle_grasp_time_str_len; i != 0; i--)
                packet_data[1258 + handle_grasp_time_str_len - i] = handle_grasp_time_str[handle_grasp_time_str_len - i];
                
            /*---------HAND GRASP THRESHOLD---------*/
            
            packet_data[1302] = TYPE_UINT32;
            packet_data[1303] = 1;
            *((uint32 *)(packet_data + 1304)) = c_mem.handle_grasp_thr;
            for(i = handle_grasp_thr_str_len; i != 0; i--)
                packet_data[1308 + handle_grasp_thr_str_len - i] = handle_grasp_thr_str[handle_grasp_thr_str_len - i];
                
                
            /*------------PARAMETERS MENU-----------*/

            for(i = input_mode_menu_len; i != 0; i--)
                packet_data[1352 + input_mode_menu_len - i] = input_mode_menu[input_mode_menu_len - i];

            for(i = control_mode_menu_len; i != 0; i--)
                packet_data[1502 + control_mode_menu_len - i] = control_mode_menu[control_mode_menu_len - i];

            for(i = yes_no_menu_len; i!= 0; i--)
                packet_data[1652 + yes_no_menu_len - i] = yes_no_menu[yes_no_menu_len - i];
...
//===================================================     set_curr_lookup_table
        ...
        break;            
//===================================================     set_handle_grasp_time
        case 26:        //handle_grasp_time - float
            g_mem.handle_grasp_time = *((float *) &g_rx.buffer[3]);
        break;
//===================================================     set_handle_grasp_thr
        case 27:        //handle_grasp_thr - uint32
            g_mem.handle_grasp_thr = *((uint32 *) &g_rx.buffer[3]);
        break; 
    }
}
