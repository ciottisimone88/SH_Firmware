...
//=========================================================     CMD_GET_CURR_DIFF
        ...
        case CMD_GET_CURR_DIFF:         //Command sent from Cuff device. The hand replies back with a filtered current
            ...
            packet_data[0] = CMD_SET_CURR_DIFF;
...
void commWrite(uint8 *packet_data, uint16 packet_lenght, uint8 next)
{
	...
	// smaller by one than the one that sends the message
...
uint8 memInit(void)
{
    ...
    g_mem.k_i           =     0 * 65536;
    g_mem.k_d           = 0.007 * 65536;  //Changed in order to avoid metallic clatter prev_val 0.2
    g_mem.k_p_c         =     1 * 65536;
    g_mem.k_i_c         =     0 * 65536;
    g_mem.k_d_c         =     0 * 65536;
